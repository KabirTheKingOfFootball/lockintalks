...
export function AuthForm({ mode, initialError = "", initialNotice = "", nextPath = "/dashboard" }: { mode: "login" | "signup"; initialError?: string; initialNotice?: string; nextPath?: string }) {
  ...
  const title = isSignup ? "Create Your Speaker Account" : "Welcome Back";

  return (
    <form action={isSignup ? "/api/auth/signup" : "/api/auth/login"} method="post" className="glass mx-auto w-full max-w-md rounded-[8px] p-6 sm:p-8">
